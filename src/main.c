...
int plot_3d(SDL_Renderer *renderer, int (*func)(double u1, double v1, double points[3]), double mat[4][4], double rgb[3]) {
    ...
    for (v = vlo; v <= vhi; v += 0.01) {
        ...
        for (u = ulo; u <= uhi; u += 0.0015) {
            ...
            // calls function, saves point into xyz, then makes necessary moves based on identity matrix and view matrix
            func(u, v, xyz);
            ...
            if ((p[0] < WIDTH && p[0] > -1) && (p[1] < HEIGHT && p[1] > -1)) {
                if (xyz[2] < zbuffer[p[0]][p[1]]) {
                    ...
                    SDL_SetRenderDrawColor(renderer, (int)(temp_rgb[0] * 255), (int)(temp_rgb[1] * 255), (int)(temp_rgb[2] * 255), 255);
...
int main() {
    ...
    // Create window
    SDL_Window *window = SDL_CreateWindow("3D Renderer", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, WIDTH, HEIGHT, SDL_WINDOW_SHOWN);
    if (!window) {
