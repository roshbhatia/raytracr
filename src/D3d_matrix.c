...
int D3d_translate(double a[4][4], double b[4][4], double dx, double dy,
                  double dz)
// a = translation*a
...
int D3d_make_movement_sequence_matrix(double mat[4][4], double inv[4][4],
                                      int num_movements,
...
// 11 - negate z...reflect in the xy plane
{
  ...
  for (i = 0; i < num_movements; i++) {
    ...
    default:
      printf("ERROR:  D3d_make_movement_sequence_matrix : unrecognized matrix "
             "type = %d\n\n",
             m);
...
int D3d_viewAA(double view[4][4], double view_inverse[4][4], double eye[3],
               double coi[3], double up[3], double a, double b, double c,
               double p, double r)
...
int D3d_viewBB(double view[4][4], double view_inverse[4][4], double eye[3],
               double coi[3], double up[3], double a, double b, double c,
               double p, double r)
// return 1 if successful, 0 otherwise
{
...
int D3d_view(double view[4][4], double view_inverse[4][4], double eye[3],
...
// return 1 if successful, 0 otherwise.
{
  ...
  if (fabs(b) < p) {
    // printf("choose AA\n") ;
    s = D3d_viewAA(view, view_inverse, eye, coi, up, a, b, c, p, r);
  } else {
    // printf("choose BB\n") ;
    s = D3d_viewBB(view, view_inverse, eye, coi, up, a, b, c, p, r);
  }

  if (s == 0)
    printf("D3d_view error\n");
